...
- **No single-letter callback parameters**: Rename cryptic abbreviations in `.map`, `.filter`, `.find`, `.reduce` callbacks. Use the domain noun the variable represents — `entry` instead of `w`, `genre` instead of `g`, `moodId` instead of `m`, `crewMember` instead of `c`. Exception: math/index variables (`i`, `x`, `y`) in tight loops where the meaning is universally understood.
...
- [ ] Do all callback parameters (`.map`, `.filter`, `.find`) use descriptive names instead of single letters?
